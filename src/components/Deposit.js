...
const Deposit = ()=> {
    ...
    const amountHandler = async(e)=>{
        if(e.target.id === 'token1'){
            ...
            const _token1Amount = ethers.utils.parseUnits(e.target.value, 'ether') //converts to wei
            const result = await amm.calculateToken2Deposit(_token1Amount)
            const _token2Amount = ethers.utils.formatUnits(result.toString(), 'ether') //converts to ether
            ...
            //fetch value from chain
            const _token2Amount = ethers.utils.parseUnits(e.target.value, 'ether') //converts to wei
            const result = await amm.calculateToken1Deposit(_token2Amount)
            const _token1Amount = ethers.utils.formatUnits(result.toString(), 'ether') //converts to ether
    ...
    return(
        ...
        <div>
            <Card style={{maxWidth:'450'}} className='mx-auto px-4'>
                {account?(
                    ...
                    style={{height:'300px'}}>Please connect wallet.
                    </p>
                )}
